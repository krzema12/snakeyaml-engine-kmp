...
private inline fun BufferedSource.sizeOfFullValidUtf8String(limitBytes: Long): Long {
    ...
    if (!request(originalSize + 1)) {
        // no more bytes left in the source, return the size
        return originalSize
    }

    // check the next byte
    // if it is not a continuation byte then bytes up to that point form a full UTF8 string
    ...
    // A valid codepoint consist of 1 leading byte and at most 3 continuation bytes (0-3)
    // https://www.rfc-editor.org/rfc/rfc3629.html#section-3
    // We go backwards to find the start of the codepoint at which we stopped
    // The marker for that is a non-continuation byte
    repeat(3) {
        size -= 1
        if (size == 0L) {
            // no more data left in the buffer
            // this means we have an invalid UTF8 string
        ...
        if (!isContinuationByte(byte)) {
            ...
            return when {
                ...
                // that has less continuation bytes then we read
    ...
    // this can only happen if the edge bytes (n-3,n-2,n-1) are all continuation
    // that would mean the UTF8 string is ill-formed
    // but, as before, this will be handled by Okio
    return originalSize
}
