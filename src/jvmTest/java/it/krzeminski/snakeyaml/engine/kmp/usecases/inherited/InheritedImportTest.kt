...
abstract class InheritedImportTest {
    ...
    @JvmOverloads
    protected fun getStreamsByExtension(
        extension: String,
    ...
    protected fun getFileByName(name: String): File =
        File("src/jvmTest/resources$PATH/$name").also {
            require(it.exists()) { "Folder not found: ${it.absolutePath}" }
            require(it.isFile)
        }

    protected fun canonicalParse(input2: InputStream, label: String): List<Event> {
    ...
    protected fun parse(input: InputStream): List<Event> {
