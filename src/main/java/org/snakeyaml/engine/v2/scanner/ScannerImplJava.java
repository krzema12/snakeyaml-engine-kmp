...
final class ScannerImplJava implements Scanner {
  ...
  void addAllTokens(List<Token> tokens) {
   ...
   */
  void fetchDocumentIndicator(boolean isDocumentStart) {
    // Set the current indentation to -1.
    unwindIndent(-1);
  ...
  void fetchFlowSequenceStart() {
    fetchFlowCollectionStart(false);
  }

  void fetchFlowMappingStart() {
   ...
   * by ']'; mappings are started by '{' and ended by '}'.
   */
  void fetchFlowCollectionEnd(boolean isMappingEnd) {
   ...
   */
  void fetchFlowScalar(ScalarStyle style) {
